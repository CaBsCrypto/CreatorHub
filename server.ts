...
app.all("/api/cron/refresh-metrics", async (req, res) => {
  ...
  try {
    ...
    if (!activeCampaigns || activeCampaigns.length === 0) {
      return res.json({ success: true, message: "No active campaigns within the 30-day window", processed: 0 });
    }

    const campaignIds = activeCampaigns.map(c => c.id);

    // Fetch up to 40 posts from these campaigns
    ...
    for (const item of posts) {
      ...
      try {
        ...
        switch (item.platform) {
          ...
          case 'instagram': 
          case 'instagram_story': data = await fetchInstagramData(item.url); break;
          case 'x':
        ...
        if (data && !data.error) {
          ...
          await supabase.from('content_metrics_history').insert({
            content_id: item.id,
...
app.post("/api/demo-request", validate(DemoRequestSchema), async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("[LEAD] Email send warning:", mailErr.message);
    }

    res.json({ success: true, message: "Solicitud recibida exitosamente" });
  } catch (error: any) {
    console.error("[LEAD] Error processing request:", error.message);
