  ...
  return (
    <motion.div
      ...
      className={`group relative glass-dark rounded-[2.5rem] shadow-2xl transition-all duration-500 border ${
        isAssigned ? 'border-emerald-500/30' : 'border-white/5'
      } hover:border-emerald-500/40 flex flex-col overflow-hidden`}
      onClick={() => onClick?.(campaign.id)}
      style={{ cursor: onClick ? 'pointer' : 'default' }}
    >
      {/* Decorative Glow */}
      <div className="absolute -right-16 -top-16 w-32 h-32 bg-emerald-500/5 rounded-full blur-3xl group-hover:bg-emerald-500/10 transition-all duration-1000" />
      ...
      {role === 'admin' && (
        <div className="absolute top-6 right-6 flex gap-2 z-10">
          <button
            ...
            className="p-2.5 rounded-xl bg-slate-900/80 text-slate-500 hover:text-emerald-400 border border-white/5 hover:border-emerald-500/30 transition-all opacity-0 group-hover:opacity-100"
          >
            <Edit2 className="h-3.5 w-3.5" />
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); onDelete(campaign.id); }}
            className="p-2.5 rounded-xl bg-slate-900/80 text-slate-500 hover:text-rose-500 border border-white/5 hover:border-rose-500/30 transition-all opacity-0 group-hover:opacity-100"
          >
      ...
      <div className="flex flex-col flex-1 p-8">
        ...
        <div className="flex items-center gap-3 mb-6 pr-20 flex-wrap">
          ...
          {onCopyLink && campaign.slug && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-white/5 rounded-full text-[9px] font-black text-slate-400 uppercase tracking-widest transition-all"
            >
        ...
        {/* Title & Description */}
        <h3 className="text-2xl font-black text-white mb-3 leading-tight tracking-tighter uppercase group-hover:text-emerald-400 transition-colors line-clamp-2">
          {campaign.name}
        </h3>
        ...
        <AnimatePresence>
          {role === 'admin' && showNotes && campaign.notes && (
            ...
            >
              <div className="p-6 bg-slate-950/50 rounded-[2rem] border border-white/5 relative group/note shadow-inner">
                ...
                <h4 className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em] mb-3 flex items-center gap-2 italic">
                  <StickyNote className="h-3.5 w-3.5" /> Internal_Directives
                </h4>
                <div className="text-sm text-slate-300 font-medium leading-relaxed whitespace-pre-wrap break-words max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                  {campaign.notes}
                </div>
              </div>
            </motion.div>
        ...
        <div className="mt-auto space-y-3 mb-8">
          <div className="flex items-center justify-between bg-slate-950/30 rounded-2xl px-5 py-4 border border-white/5 group-hover:border-emerald-500/20 transition-all duration-500">
          ...
          <div className="flex items-center justify-between bg-slate-950/30 rounded-2xl px-5 py-4 border border-white/5 group-hover:border-emerald-500/20 transition-all duration-500">
            ...
            <span className="text-xl font-black text-emerald-500 tabular-nums tracking-tighter">
              {totalPosts}
            </span>
          </div>
          <div className={`flex items-center justify-between rounded-2xl px-5 py-4 border transition-all duration-500 ${
            role === 'admin' ? 'bg-emerald-600 border-white/20' : 'bg-slate-900 border-white/5'
          }`}>
        ...
        <div className="pt-6 border-t border-white/5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            {onViewReport && (
              <button
                ...
                className="px-5 py-2.5 bg-slate-900 hover:bg-rose-600 text-slate-400 hover:text-white text-[10px] font-black uppercase tracking-widest rounded-xl border border-white/5 hover:border-rose-500/30 transition-all shadow-xl"
              >
                Analytics
              </button>
            )}
            ...
            {campaign.share_token && onCopyLink && (
              <button
                ...
                className="p-3 bg-slate-900 text-slate-500 hover:text-emerald-400 rounded-xl border border-white/5 hover:border-emerald-500/20 transition-all shadow-xl"
              >
          ...
          <button
            ...
            className={`flex-1 px-6 py-3 text-slate-950 text-[10px] font-black uppercase tracking-[0.3em] rounded-xl transition-all shadow-2xl active:scale-95 ${
              role === 'admin' ? 'bg-white hover:bg-slate-200' : 'bg-emerald-500 hover:bg-emerald-400'
            }`}
          >
