  ...
  if (filteredUsers.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-40 animate-in fade-in duration-1000">
        <div className="w-24 h-24 glass-dark rounded-[2rem] flex items-center justify-center mb-6 border border-slate-200">
        ...
        {filteredUsers.map((u, i) => (
          <motion.div 
            ...
            className={`glass-dark p-8 rounded-[3rem] border hover:shadow-[0_20px_60px_rgba(0,0,0,0.5)] hover:-translate-y-2 transition-all duration-500 group cursor-pointer relative overflow-hidden ${
              u.role === 'admin' ? 'border-indigo-200' :
            ...
            <div className="relative z-10">
              <div className="flex items-start justify-between mb-10">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-2xl group-hover:scale-110 group-hover:rotate-6 ${
                  u.role === 'admin' ? 'bg-white border border-indigo-200 text-indigo-600' :
                  u.role === 'manager' ? 'bg-white border border-cyan-500/20 text-cyan-400' :
              ...
              <h3 className="text-2xl font-black text-white mb-2 group-hover:text-indigo-600 transition-colors tracking-tighter uppercase">
              ...
              <div className="pt-8 border-t border-slate-200 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em] opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0 italic">
