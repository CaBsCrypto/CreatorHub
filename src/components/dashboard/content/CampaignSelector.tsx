  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center justify-between w-full rounded-xl border border-slate-200 bg-gray-50/50 py-2.5 px-4 text-sm font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all outline-none text-left shadow-sm"
      >
        <span className={selectedCampaignId ? 'text-slate-900 font-bold' : 'text-slate-500 italic'}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-[110] left-0 right-0 mt-2 bg-slate-50 rounded-2xl shadow-2xl border border-slate-200 overflow-hidden ring-1 ring-white/10"
          >
            <div className="p-2 border-b border-slate-100 flex items-center gap-2 sticky top-0 bg-white/80 backdrop-blur-md">
              ...
              <input
                ...
                className="w-full bg-transparent border-none text-[10px] font-black uppercase tracking-widest text-slate-900 placeholder:text-slate-300 focus:ring-0 outline-none py-1"
              />
            </div>
