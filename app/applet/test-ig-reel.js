...
async function testReel() {
  ...
  try {
    ...
    if (response.data.items && response.data.items.length > 0) {
        ...
        console.log("Item view_count:", item.view_count);
        console.log("Item play_count:", item.play_count);
    } else if (response.data.data) {
        console.log("Data keys:", Object.keys(response.data.data));
    } else {
        console.log("Root view_count:", response.data.view_count);
        console.log("Root play_count:", response.data.play_count);
    }
